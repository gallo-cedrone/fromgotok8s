...
push-app:
	...
	helm upgrade fromgotok8s-${TRAVIS_BRANCH}-${TRAVIS_TAG} static-gallo-cedrone-repo/fromgotok8s --install --namespace ${TRAVIS_BRANCH} --set image.version=${TRAVIS_BRANCH}-${TRAVIS_TAG}
...
.PHONY: changelog compile test integration-test image push-image push-app changelog add-repo set-cluster tools-and-vars
